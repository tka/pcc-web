class AddTenderInfoCounts < ActiveRecord::Migration
  def up
    ...
    ActiveRecord::Base.connection.execute('update tenderers set winning_tender_info_count = (select count(*) from tender_infos where tender_infos.tenderer_id = tenderers.id and winning = 1)')
  end
